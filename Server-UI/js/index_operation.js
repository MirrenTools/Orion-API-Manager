...
function loadProjectInfo(data) {
    ...
    var txt =
        ...
        ' <span class="badge"> ' + data.versions + '</span>' +
    ...
    txt += '<i class="fond-size-16 mleft10px">最后操作时间: ' + new Date(data.lastTime).toLocaleString().replace(/:\d{1,2}$/, ' ') + '</i>';
    txt += '</h1>';
    ...
    if (data.externalDocs != null) {
        ...
        if (!jQuery.isEmptyObject(externalDocs)) {
            txt += '<h4>拓展文档: <a target="_blank" href="' + externalDocs.url + '"><span>' + externalDocs.description + '</span></a></h4>';
        }
    }
    if (data.contactName != null && data.contactName != '') {
        txt += '<h4></i>联系人: <span>' + data.contactName + '</span><h4>';
    }
    if (data.contactInfo != null && data.contactInfo != '') {
        txt += '<h4></i>联系信息: <span>' + data.contactInfo + '</span><h4>';
    }
    txt += '<a href=\'' + SERVER_HOST + '/project/downJson/' + data.key + '\'>下载.json文件</a>';
    var fileURL = SERVER_HOST + '/project/getJson/' + data.key;
    txt += '<a style="margin-left: 15px;" href=\'' + SERVER_HOST + "/Client-UI/index.html?file=" + fileURL +"&id="+data.key+ '\' target="_blank">在Client-UI中查看</a>';
    ...
    for (var it = 0; it < items.length; it++) {
        ...
        if (data.apis != null && data.apis.length > 0) {
            ...
            for (var i = 0; i < apis.length; i++) {
                ...
                txt += '<div class="opblock-summary-description">' + opblack.title + '</div>';
                txt += '</div>';
                //opblock-summary end
                ...
                txt +=
                    ...
                    '<a class="btn btn-sm btn-default" href="./updateApi.html?gid=' + data.groupId + '&pid=' + data.projectId + '&aid=' + opblack.apiId + '">修改</a>' +
                    '<span class="btn btn-sm btn-danger mleft10px" onclick="deleteApi(\'' + opblack.apiId + '\')">删除</span>' +
                    '</div>' +
                    '</div>';
                //接口操作 end
                //接口描述 begin
                ...
                if (opblack.responses != null) {
                    ...
                    if (responses.length > 0) {
                        txt +=
                            '<div class="table-responsive plr20px"><table class="table table-bordered">' +
                            '<thead><tr><th width="15%" class="text-align-center">参数类型</th><th width="20%">参数名称</th><th>参数描述</th></tr></thead><tbody>';
                        ve_ps = responses;
                        for (var vep = 0; vep < ve_ps.length; vep++) {
                            txt += '<tr><td class="text-align-center">' + ve_ps[vep].type + '</td><td>' + ve_ps[vep].name + '</td><td>' + ve_ps[vep].description + '</td>';
                ...
                //返回结果 end
                //附加说明 begin
                if (opblack.additional != null) {
                    var ais = JSON.parse(opblack.additional);
                    if (ais.length > 0) {
                        txt +=
                            '<div class="responses-wrapper">' +
                            '<div class="opblock-section-header">' +
