...
function getApiInfo() {
    ...
    if (additionals.length > 0) {
        data.additional = JSON.stringify(additionals);
    }
    //附加说明 end
    return data;
}
        ...
        if (result.code == 200) {
            ...
            if (data.responses != null) {
                ...
                for (var r = 0; r < resp.length; r++) {
                    $("#response_tablebody").append($(getResponseParameterTableHtml(resp[r].type, resp[r].name, resp[r].description, resp[r].items)));

                }
            }

            if (data.additional != null) {
                var ais = JSON.parse(data.additional);
